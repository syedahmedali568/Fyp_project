    ...
    def _create_kb(self):
        try:
            reader = SimpleDirectoryReader(
                input_files=["./rag/knowledgeBase.pdf"]
            )
            documents = reader.load_data()
            chroma_collection = self._chroma_client.get_or_create_collection("kitchen_db")
            vector_store = ChromaVectorStore(chroma_collection=chroma_collection)
            storage_context = StorageContext.from_defaults(vector_store=vector_store)
            self._index = VectorStoreIndex.from_documents(
                documents, service_context=self._service_context, storage_context=storage_context
            )
            print("Knowledgebase created successfully with ChromaDB!")
